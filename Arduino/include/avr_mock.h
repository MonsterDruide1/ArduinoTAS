...
bool Endpoint_ConfigureEndpoint(int val1, int val2, int val3, int val4) {
	return true;
}
...
int Endpoint_Read_Stream_LE(void* const buf, size_t len, uint16_t* const bytesProcessed) {
...
int Endpoint_Write_Stream_LE(void* const buf, size_t len, uint16_t* const bytesProcessed) {
	return ENDPOINT_RWSTREAM_NoError;
}
